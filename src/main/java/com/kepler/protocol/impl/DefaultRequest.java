...
public class DefaultRequest implements Request {
	...
	public DefaultRequest(Request request, Integer ack) {
		this(ack, request.headers(), request.service(), request.method(), request.async(), request.args(), request.types(), request.serial());
	}

	public DefaultRequest(Request request, Integer ack, boolean async) {
		this(ack, request.headers(), request.service(), request.method(), async, request.args(), request.types(), request.serial());
	}

	public DefaultRequest(Request request, Integer ack, Object[] args) {
		this(ack, request.headers(), request.service(), request.method(), request.async(), args, request.types(), request.serial());
	}
